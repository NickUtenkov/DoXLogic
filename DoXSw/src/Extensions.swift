...
extension String
{
	...
	var length:Int
	{
		//return self.characters.count//Swift 3
		return self.count
	}
...
extension UIView
{
	...
	func dumpHierarchy(_ level:inout Int)
	{
		var str = ""
		for _ in 0..<level
		{
		...
		str += String(format:"%@%@ (%.0f %.0f,%.0f %.0f)",hasAmbiguousLayout ? "*" : "",String(describing:type(of:self)),frame.origin.x,frame.origin.y,frame.size.width,frame.size.height)
		print(str)
		level += 1
		let arView = subviews
		for pView in arView
		{
			pView.dumpHierarchy(&level)
		}
		level -= 1
	}
}
...
class TranslateResizeMaskToConstraints: UIView
{
	...
	{
		super.awakeFromNib()
		translatesAutoresizingMaskIntoConstraints = (Enabled || _interfaceBuilderContext)
	}
